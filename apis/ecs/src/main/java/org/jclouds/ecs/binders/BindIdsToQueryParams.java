 ...
 */
package org.jclouds.ecs.binders;

import static com.google.common.base.Functions.toStringFunction;
import static com.google.common.base.Preconditions.checkNotNull;
import static com.google.common.collect.Iterables.transform;
import static org.jclouds.ecs.reference.EcsQueryParams.ID_KEY;
...
public class BindIdsToQueryParams implements Binder {
	...
	/**
	 * Binds the ids to query parameters. The pattern, as specified by GoGrid's
	 * specification, is:
	 * 
	...
	public <R extends HttpRequest> R bindToRequest(R request, Object input) {
		...
		if (checkNotNull(input, "input is null") instanceof Long[]) {
			...
			return (R) request
					...
					.replaceQueryParam(
							ID_KEY,
							transform(ImmutableList.copyOf(names),
									toStringFunction())).build();
		} else if (input instanceof long[]) {
			long[] names = (long[]) input;
			return (R) request
					.toBuilder()
					.replaceQueryParam(ID_KEY,
							transform(Longs.asList(names), toStringFunction()))
					.build();
		} else {
